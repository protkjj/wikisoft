"""
학습 API: 사용자 피드백으로부터 규칙 학습 (WIKISOFT4)

WIKISOFT3에서 마이그레이션됨
- 인증: verify_token → require_auth (JWT)
...
from api.v4.routes.auth import require_auth, get_current_user
from core.security.auth import User
from core.ai.knowledge_base import (
    add_error_rule,
...
async def delete_rule(
    ...
    if success:
        return {"status": "deleted", "rule_id": rule_id, "deleted_by": user.id}
    else:
